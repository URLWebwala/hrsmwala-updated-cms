...
class ProjectBugController extends Controller
{
    ...
    public function store(StoreProjectBugRequest $request)
    {
        if (Auth::user()->can('create-project-bug')) {
            ...
            $assignedUsers = is_array($validated['assigned_to']) ? $validated['assigned_to'] : (json_decode($validated['assigned_to'], true) ?? []);
            foreach ($assignedUsers as $assignedUserId) {
                if ((int) $assignedUserId === (int) Auth::id()) {
                    continue;
                }
                app(NotificationService::class)->sendToUser((int) $assignedUserId, [
                    'type' => 'task_assigned',
                    'title' => 'New bug assigned',
                    'message' => 'You have been assigned bug: ' . $bug->title,
                    'item_type' => 'bug',
                    'item_id' => $bug->id,
                    'meta' => ['project_id' => $bug->project_id, 'priority' => $bug->priority],
                ]);
            }
    ...
    public function update(UpdateProjectBugRequest $request, ProjectBug $bug)
    {
        ...
        if (Auth::user()->can('edit-project-bug') && $canEdit) {
            ...
            $assignedUsers = isset($validated['assigned_to'])
                ? (is_array($validated['assigned_to']) ? $validated['assigned_to'] : (json_decode($validated['assigned_to'], true) ?? []))
                : [];
            foreach ($assignedUsers as $assignedUserId) {
                if ((int) $assignedUserId === (int) Auth::id()) {
                    continue;
                ...
                app(NotificationService::class)->sendToUser((int) $assignedUserId, [
                    'type' => 'task_assigned',
                    'title' => 'Bug assignment updated',
                    'message' => 'Bug assigned/updated: ' . $bug->title,
                    'item_type' => 'bug',
                    'item_id' => $bug->id,
                    'meta' => ['project_id' => $bug->project_id, 'priority' => $bug->priority],
                ]);
    ...
    public function move(Request $request, ProjectBug $bug)
    {
        ...
        if ($canMove) {
            ...
            app(NotificationService::class)->sendToRole('admin', [
                ...
                'message' => 'Bug "' . $bug->title . '" moved to ' . ($newStage?->name ?? 'new stage'),
                'item_type' => 'bug',
                'item_id' => $bug->id,
