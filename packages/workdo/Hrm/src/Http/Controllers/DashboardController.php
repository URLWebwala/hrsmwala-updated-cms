    ...
    private function employeeDashboard(Request $request)
    {
        ...
        if ($todayAttendance && $todayAttendance->clock_in && !$todayAttendance->clock_out) {
            ...
            if ($shift) {
                ...
                // If no shift is assigned, just check if it's a recent pending attendance (e.g. within 24 hours)
                $clockInDateTime = Carbon::parse($pendingAttendance->clock_in);
                if ($clockInDateTime->diffInHours(now()) < 24) {
                    $isCurrentlyClockedIn = true;
                    $activeAttendance = $pendingAttendance;
                }
            }
