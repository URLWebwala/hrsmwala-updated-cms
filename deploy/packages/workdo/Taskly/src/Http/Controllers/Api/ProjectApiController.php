...
class ProjectApiController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            if (Auth::user()->can('manage-project')) {
                $items = Project::query()
                    ->select('id', 'name', 'description', 'budget', 'start_date', 'end_date', 'status', 'created_by', 'tracker_enabled')
                ...
                $items->getCollection()->transform(function ($project) {
                    ...
                    return [
                        ...
                        'tracker_enabled' => (bool)$project->tracker_enabled,
                        'members'        => $members,
                        'clients'        => $clients,
                    ];
                });

                $trackerItems = $items->items();
                $pagination = [
                    'total'        => $items->total(),
                ...
                // Tracker desktop app (Dart client) expects projects as a direct list in `data`.
                if (str_contains((string) $request->userAgent(), 'Dart')) {
    ...
    private function createProject(Request $request)
    {
        if (Auth::user()->can('create-project')) {
            $validator = Validator::make($request->all(), [
                ...
                'tracker_enabled' => 'nullable|boolean',
            ]);
            ...
            // Default to tracker-enabled unless explicitly disabled by caller.
            $project->tracker_enabled = $request->has('tracker_enabled') ? (bool) $request->tracker_enabled : true;
    ...
    private function updateProject(Request $request)
    {
        if (Auth::user()->can('edit-project')) {
            ...
            if (isset($request->tracker_enabled)) {
                $project->tracker_enabled = $request->tracker_enabled;
            }
            $project->save();
