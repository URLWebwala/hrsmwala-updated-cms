                                ...
                                {filteredModules.map((module) => (
                                    <div key={module.module} className="flex items-center gap-3 p-4 border rounded hover:bg-muted/50">
                                        <img
                                            src={
                                                module.image ||
                                                getPackageFavicon(module.module) ||
                                                `/packages/workdo/${module.module}/favicon.png`
                                            }
                                            alt=""
                                            className="w-8 h-8 border rounded"
                                            onError={(e) => {
                                                // If module favicon is missing (common for new packages),
                                                // replace with a tiny inline SVG so UI never looks broken.
                                                const fallbackSvg = encodeURIComponent(`
                                                  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                                                    <rect x="1" y="1" width="30" height="30" rx="6" fill="#0ea5e9"/>
                                                    <path d="M10 12h12v8H10z" fill="white" opacity="0.95"/>
                                                    <path d="M13 22h6" stroke="white" stroke-width="2" stroke-linecap="round"/>
                                                    <path d="M12 10l2-2h4l2 2" stroke="white" stroke-width="2" stroke-linecap="round" fill="none"/>
                                                  </svg>
                                                `);
                                                (e.currentTarget as HTMLImageElement).src = `data:image/svg+xml;charset=utf-8,${fallbackSvg}`;
                                            }}
                                        />
                                        <span className="text-sm truncate flex-1">{module.alias || getPackageAlias(module.module)}</span>
                                        <Checkbox
