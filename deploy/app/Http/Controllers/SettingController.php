...
class SettingController extends Controller
{
    ...
    public function updateTrackerAppSettings(Request $request)
    {
        if (Auth::user()->hasRole('superadmin')) {
            $request->validate([
                'tracker_app' => 'required|file|mimes:exe,zip,dmg|max:51200', // 50MB
            ]);

            if ($request->hasFile('tracker_app')) {
                $file = $request->file('tracker_app');
                $fileName = $file->getClientOriginalName();
                $path = $file->storeAs('tracker-apps', $fileName, 'public');

                setSetting('tracker_app_file', $path);
                setSetting('tracker_app_name', $fileName);

                return back()->with('success', __('Tracker app uploaded successfully.'));
            }
        }
